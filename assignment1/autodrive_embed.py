...
class ShouldntHappen(Exception):
  pass

TimeoutException = ShouldntHappen # will be overwritten if needed...
...
def run(gdict,answer,answerFactory,errlog,grabPlots=False):
  # Override plot to force save to png if grabPlots;
  # Use timeout as such if >0.
  ...
  timeout=int(sys.argv[2][10:] if ((len(sys.argv) > 2)
                                   and (sys.argv[2][:10]=='--timeout='))
              else '0')
  if timeout>0:
    import stopit, logging
    global TimeoutException
    TimeoutException=stopit.TimeoutException
    logging.getLogger('stopit').setLevel(logging.ERROR)
    cc=False
    with stopit.ThreadingTimeout(timeout) as cc:
      _run(gdict,answer,answerFactory,errlog)
    if not cc:
      # Timed out
      print('Timed out after %s seconds'%timeout,file=errlog)
      errlog.flush()
      exit(1)
  else:
    _run(gdict,answer,answerFactory,errlog)
...
def _run(gdict,answer,answerFactory,errlog):
  global counter
      ...
      for a in aLines:
        ...
        if (multi is not None) and (not mbs) and prefix.match(a):
          ...
          try:
            ...
            errs+=1
            print("""The following lines threw a TimeoutException:%s
  -------"""%multi, file=errlog)
            raise
          except Exception as e:
            errs+=1
  ...
  -------"""%(e.__class__.__name__,bogus,multi),
        ...
        try:
          ...
          errs+=1
          print("""The following line threw a TimeoutException:%s
  -------"""%a, file=errlog)
          raise
        except Exception as e:
          errs+=1
